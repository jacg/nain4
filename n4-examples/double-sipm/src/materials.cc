...
G4Material* csi_with_properties(const config& config) {
    ...
    auto cold = false;
    auto csi_scint_yield = cold ? 50000 / MeV : config.csi_scint_yield;
    auto csi_time_fast   = cold ? 1015 * ns   :  6 * ns;
    auto csi_time_slow   = cold ? 1015 * ns   : 28 * ns; // only one component at cold temps!
