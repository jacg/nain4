...
G4PVPlacement* make_geometry(data& data, const config& config) {
    ...
    auto plastic = n4::material("G4_POLYCARBONATE");
    ...
    n4::place(source_ring).in(world).rot_y(90*deg).now();
    ...
    auto scintillator = n4::box("Scintillator").xy(scint_xy                 ).z(scint_z               ).place (csi)
        .in(coating_log).at_z(coating_thck/2).now();

    // Make a reusable (place many times) logical volume, containing:
    // scintillator, coating and border-surface-between-them
    ...
    n4::place(coated_scint_log).in(world).at_z(scintillator_offset).rot_y(  0*deg).copy_no(0).now();
    n4::place(coated_scint_log).in(world).at_z(scintillator_offset).rot_y(180*deg).copy_no(1).now();
